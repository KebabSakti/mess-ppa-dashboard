...
function EmployeeEditPage(props: any) {
  const { id } = useParams();
  const navigate: any = useNavigate();
  const [single, setSingle] = useState<StateEntity<EmployeeEntity>>({
    loading: true,
    data: null,
  });
  ...
  function formOnSubmit(event: any) {
    ...
    if (event.target["password"].value.length > 0) {
      inputs = { ...inputs, password: event.target["password"].value };
    }
                ...
                <form className="space-y-4" onSubmit={formOnSubmit}>
                  ...
                  <div className="flex flex-col space-y-2">
                    ...
                    <input
                      ...
                      type="text"
                      className="input input-bordered w-full max-w-xs"
                      defaultValue={single.data?.phone}
                    />
                  </div>
                  <div className="flex flex-col space-y-2">
                    <label>Aktif</label>
